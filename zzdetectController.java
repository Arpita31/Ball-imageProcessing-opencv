...
public class zzdetectController {
			...
			private Image grabFrame() { 
				...
				// convert the frame to HSV
							...
							//for yellow ball detection hsv values
							 Scalar min = new Scalar(23,41,133);//(20, 100, 100);//BGR-A
							 Scalar max= new Scalar(40,150,255);//(30, 255, 255);//BGR-A
							Core.inRange(hsvImage, min, max, mask);
							...
							Mat erodeElement = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(35, 35));
			...
			private Mat findAndDrawBalls(Mat maskedImage, Mat frame) {
				...
				if (hierarchy.size().height > 0 && hierarchy.size().width > 0) {
					...
					for (int idx = 0; idx >= 0; idx = (int) hierarchy.get(0, idx)[0]) { 
						...
						Imgproc.rectangle(frame, rect.tl(), rect.br(), new Scalar(250, 0, 0), 2 );
		 ...
		 *   @return the {@link Image} to show */ 
			...
			public void setRootElement(BorderPane root) {
